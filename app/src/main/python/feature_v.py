# -*- coding: utf-8 -*-
"""
Created on Tue May 10 17:47:13 2022
...
def feature_v(path):
    try:
        #a = pd.read_csv(name+'\\unlockcha\\'+filename+'.csv')
        #filename = join(dirname(__file__), "unlock/han_v7.csv")
        filename = join(dirname(__file__), path)
        a = pd.read_csv(filename)
        signals, info = nk.ecg_process(a['Lead2'], sampling_rate=1000)
        _, peaks = nk.ecg_delineate(signals['ECG_Clean'][0:5000])
        infos = nk.ecg_findpeaks(signals['ECG_Clean'][0:5000])
